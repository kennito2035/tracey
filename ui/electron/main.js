 ...
 */
/* The RENDERER confirms before calling this, in an in-app dialog. It was briefly
 * done here with dialog.showMessageBox and that is why the note above exists:
 * while the core runs, RPIT takes the raw pen and the injected HOVER pointer is
 * never promoted to a mouse message for a window that does not speak WM_POINTER,
 * so a native prompt sat there with no visible cursor. Nothing else invokes this
 * channel, so the confirm belongs where the cursor works. */
